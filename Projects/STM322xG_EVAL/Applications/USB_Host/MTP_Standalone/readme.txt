...
USB host application based on the Media Transfer Protocol (MTP) on the STM32F2xx devices.
...
It's worth noting that the system clock (SYSCLK) can be configured, depending on the used USB Core:
 - SYSCLK is set to 120 MHz: for FS Core because used embedded PHY requires 48 MHz clock,
                             achieved only when system clock is set to 120 MHz.                             
 - SYSCLK is set to 120 MHz: for only HS Core, since no embedded PHY is used.
...
@par Directory contents
  ...
  - USB_Host/MTP_Standalone/Src/system_stm32f2xx.c      STM32F2xx system clock configuration file
  - USB_Host/MTP_Standalone/Src/stm32f2xx_it.c          Interrupt handlers
  ...
  - USB_Host/MTP_Standalone/Inc/stm32f2xx_it.h          Interrupt handlers header file
  - USB_Host/MTP_Standalone/Inc/lcd_log_conf.h          LCD log configuration file
  - USB_Host/MTP_Standalone/Inc/stm32f2xx_hal_conf.h    HAL configuration file
...
@par Hardware and Software environment
  ...
  - STM322xG-EVAL RevC Set-up
    - Plug the MTP device into the STM322xG-EVAL board through 'USB micro A-Male 
      to A-Female' cable to the connector:
      - CN9 : to use USB High Speed (HS) with embedded PHY(U7)
      - CN14: to use USB Full Speed (FS)
    - Supply the board with an external power (connector CN15)
